...
describe('signup and credentials', () => {
  it('normalizes usernames, hashes passwords, and clears demo progress', async (context) => {
    const { app, store } = await testApp()
    context.after(() => app.close())
    const demo = tracker({ isDemo: true })
    ...
    assert.deepEqual(body.tracker.habits, demo.habits)
    assert.deepEqual(body.tracker.completions, {})
    assert.equal(body.tracker.isDemo, false)
...
describe('tracker validation and updates', () => {
  it('updates a valid tracker and rejects strict-shape violations', async (context) => {
    const { app } = await testApp()
    context.after(() => app.close())
    const token = (await signup(app)).json().token as string
    const next = tracker({ title: 'A New Chapter', completions: { '2026-09-03': ['read'] } })
    ...
    const invalidTrackers: unknown[] = [
      ...
      { ...next, habits: [{ id: 'move', name: ' Move ' }] },
      { ...next, completions: { '2026-02-29': ['move'] } },
      { ...next, completions: { '2026-09-01': ['move', 'move'] } },
      { ...next, completions: { '2026-09-01': ['missing'] } },
    ]
...
describe('public profiles and privacy', () => {
  it('searches public username prefixes and returns matching month statistics', async (context) => {
    const { app, store } = await testApp()
    context.after(() => app.close())
    ...
    const profile = await app.inject({ method: 'GET', url: '/v1/profiles/alice?month=2026-09' })
    ...
    assert.deepEqual(profile.json(), {
      profile: {
        ...
        habits: ['Move', 'Read'],
        month: '2026-09',
        stats: { completed: 3, total: 60, percentage: 5 },
